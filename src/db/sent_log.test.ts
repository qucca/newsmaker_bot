...
import { insertSent } from './sent_log.js';
...
test('insertSent: true на новую строку, false на повтор (дедуп — сигнал для счётчика)', () => {
  const db = memDb();
  const { chatId, clusterId } = seedUserAndCluster(db);

  assert.equal(insertSent(db, chatId, clusterId, 'digest', 100), true);
  assert.equal(insertSent(db, chatId, clusterId, 'digest', 200), false); // ON CONFLICT — не вставка
});
