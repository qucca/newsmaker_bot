...
const SYSTEM_TEXT = [
  ...
  '- tags: 0..4 topic tags chosen ONLY from this fixed list. Prefer the MOST SPECIFIC',
  '  matching topic; if a story belongs to a group but no specific subtopic fits, use that',
  "  group's `*_other` tag. Omit tags entirely only if nothing fits:",
  `  ${CATEGORIES.join(', ')}.`,
  ...
  '- regions: 0..4 ISO-3166-1 alpha-2 country codes (UPPERCASE) the story is genuinely ABOUT,',
  '  most central first. Be conservative: only countries the events/subjects belong to.',
  '  For borderless topics (global tech releases, science, markets with no national locus) return [].',
