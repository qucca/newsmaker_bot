...
test('runGlobalPass: возвращает упавшие шаги (сигнал для алертинга)', async () => {
  const ok = await runGlobalPass({
    logger: silent,
    collect: async () => {},
    enrich: async () => {},
    cluster: async () => {},
  ...
  assert.deepEqual(bad.failed, ['collect', 'cluster']);
});
